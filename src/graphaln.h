...
enum SearchRes { OPTIMAL = 0 , EXHAUSTED = -1, MULTIPLE = 1 };
...
public:
         ...
         */
        GraphAligner(const DBGraph& dbg,  size_t readCorrDFSNodeLimit) : dbg(dbg), maxVisits(readCorrDFSNodeLimit),
                aligner(2, 1, -1, -3) {}
        ...
        void setMaxVisits(size_t maxVisits) {
         ...
         SearchRes DFSAln_guidedbyMarkov(const string& P, const NodePosPair& srcNPP,
                                 GraphAlignment& bestGA, bool markovFilter, bool branchAndBoundFileter, vector<NodeID>seedNodeChain,  const MarkovChainHandler &mch);
...
class MarkovGraphAligner :public  GraphAligner  {
        const DBGraph& dbg;     // const-ref to the underlying dbg
        size_t maxVisits;       // maximum number of nodes visited
        MarkovChainHandler &mch; // markov chain transition matrix
public :
         ...
         * @param readCorrDFSNodeLimit max number of visited nodes before give up in dfs search
         */
        MarkovGraphAligner(const DBGraph& dbg, MarkovChainHandler& mch_, size_t readCorrDFSNodeLimit) : GraphAligner(dbg,readCorrDFSNodeLimit), dbg(dbg), maxVisits(readCorrDFSNodeLimit),
        mch(mch_) {}
        ...
        SearchRes DFSAln_guided(const string& P, const NodePosPair& srcNPP,
                                 GraphAlignment& bestGA, bool markovFilter, bool branchAndBoundFileter, vector<NodeID>seedNodeChain){
                                         return( DFSAln_guidedbyMarkov( P,srcNPP,bestGA,  markovFilter,  branchAndBoundFileter, seedNodeChain, mch));
                                }
};
