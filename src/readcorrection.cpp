...
void ReadCorrection::validateSeeds( vector<Seed>& mergedSeeds, vector<NodePosPair>& nppv){
        vector<Seed> validSeeds;
        for (auto seed : mergedSeeds) {
                if (seed.nodeID.size () == 1 ){
                        validSeeds.push_back(seed);
                        continue;
                }
                vector< vector<NodeID> > validSubchains;
                if (mch.validateChain(seed.nodeID, validSubchains ) ){
                        validSeeds.push_back(seed);
                        continue;
                }
                // lets take the longest valid subchain
                if (validSubchains.size() == 0)
                        continue;
                vector<NodeID> validChain = validSubchains[0];
                if (validChain.size()== 0)
                        continue;
                int start = 0;
                bool multiple = false;
                for (int k = 0; k < seed.nodeID.size() -validChain.size ()+1; k ++){
                        bool match = true;
                        for (int j = 0; j < validChain.size() ; j++){
                                if (seed.nodeID[k+j] != validChain[j]){
                                        match = false;
                                        break;
                                }
                        }
                ...
                int i = seed.readFirst;
                vector <NodeID> visited ;
                while (i < seed.readEnd ){
              
                        if( nppv[i].isValid() && ( visited.size() ==0  ||   visited.back ()!= nppv[i].getNodeID() )){
                                visited.push_back(nppv[i].getNodeID());
                                if (visited.size() > start){
                                        break;
                                }
                        }
                        i ++;        
                }
                Seed newSeed(nppv[i].getNodeID(), nppv[i].getPosition(), i, i + 1);
                int j = 1;
                while (i < seed.readEnd  ){
                        if (newSeed.nodeID.back () != nppv[i].getNodeID()&& nppv[i].isValid()){
                                newSeed.nodeID.push_back(nppv[i].getNodeID());
                                j = j+1;
                        }
                        if (j == validChain.size())
                                break;
                        i ++;
                }
                
          
                size_t OLSize = dbg.getSSNode( nppv[i].getNodeID()).getMarginalLength() + i   ;
                if (j == 1)
                        OLSize = OLSize - newSeed.nodeFirst;
                if (OLSize > seed.readEnd)
                        OLSize  = seed.readEnd;
                newSeed.readEnd = OLSize;
                
                /*int lastVisit = i ;
                for (int j = i ; j<seed.readEnd ; j++ ){
                       if (!nppv[j].isValid())
                                continue;
                       if (nppv[j].getNodeID() != validChain.back())
                               break;
                       if ( nppv[j].getNodeID() == validChain.back())
                               lastVisit = j;
                }
               
                newSeed.readEnd = lastVisit + 1 ;
                if (newSeed.readEnd != OLSize){
                        int stop =0;
                        stop ++;
                }*/
                
                validSeeds.push_back(newSeed);
        }
        mergedSeeds = validSeeds;
}
...
SearchRes ReadCorrection::extendSeedSlow (string& read, vector<NodePosPair>& npp,
        ...
        //SearchRes res = graphaln.DFSAln_guided(read.substr(readStart), NodePosPair(nodeID, nodeStart), ga, branchAndBound, nodeChain);
...
void ReadCorrection::correctReadInOneStep(ReadRecord& record,
                                          AlignmentMetrics& metrics)
{
        ...
        if (read.length() < Kmer::getK()){
                ...
                record.preRead.insert(record.preRead.length()-1,"\tA(0)A");
                return;
        }
        ...
        //cout <<"original:\t"<< read <<endl;
        //cout <<"corrected:\t" <<bestCorrectedRead <<endl;
        ...
        string insertedStr = makeAlignmentInfo(branch, bestScore, bestNodeChain, nodePos, correctionStatus);
        ...
        if (  bestScore > minSim && correctionStatus != EXHAUSTED && dbg.validateChain(bestNodeChain)) {
                insertedStr =  "\tA(1)A\t" + insertedStr;
                record.preRead.insert(record.preRead.length()-1,"\tA(1)A\t");
...
string ReadCorrection::makeAlignmentInfo(bool branch, int bestScore, vector<NodeID> bestNodeChain, pair<size_t, size_t> nodePos, SearchRes correctionStatus ){
        ...
        switch(correctionStatus){
                case 0 :
                        insertedStr = insertedStr + "\tOPTIMAL";
                        break;
                case 1 :
                        insertedStr = insertedStr + "\tMULTIPLE";
                        break;
                case -1 :
                         insertedStr = insertedStr + "\tEXHAUSTED";
                        break;
        }        
        return insertedStr;
}
...
void ReadCorrection::correctChunkInOneStep(vector<ReadRecord>& readChunk,
                                     AlignmentMetrics& metrics)
{
        for (auto& it : readChunk)
                correctReadInOneStep(it, metrics);
}
...
void ReadCorrectionHandler::workerThread(size_t myID, LibraryContainer& libraries,
                                         AlignmentMetrics& metrics)
{
        ReadCorrection readCorrection(dbg, settings, *sa, startpos, mch, false, branchAndBound,useEssaMem );
...
void ReadCorrectionHandler::workerThreadSingleStep(size_t myID, LibraryContainer& libraries,
                                         AlignmentMetrics& metrics)
{
        ReadCorrection readCorrection(dbg, settings, *sa, startpos, mch, false, branchAndBound , useEssaMem);
        ...
        while (true) {
                ...
                readCorrection.correctChunkInOneStep(myReadBuf, threadMetrics);
...
void ReadCorrectionHandler::saveMetricsInFile(AlignmentMetrics &metrics){
        ...
        myfile <<"kmerSize:"         << settings.getK()<<endl;
...
void ReadCorrectionHandler::doErrorCorrectionSingleStep(LibraryContainer& libraries)
{
        ...
        for (size_t i = 0; i < workerThreads.size(); i++)
                workerThreads[i] = thread(&ReadCorrectionHandler::workerThreadSingleStep,
...
ReadCorrectionHandler::ReadCorrectionHandler(DBGraph& g, const Settings& s,bool markov_filter_ , bool useEssaMem_) :
dbg(g), settings(s), sa(NULL),mch(dbg,settings), markov_filter(markov_filter_), branchAndBound(settings.getBranchAndBound()), useEssaMem(useEssaMem_)
{
        Util::startChrono();
                                         ...
                                         AlignmentMetrics& metrics)
{
        ...
        ReadCorrection readCorrection(dbg, settings, *sa, startpos, mch, markov_filter, branchAndBound,useEssaMem);
