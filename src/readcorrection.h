...
private:
         ...
         * 
         **/
        void correctReadInOneStep(ReadRecord& record,
                                    AlignmentMetrics& metrics);
        
        string makeAlignmentInfo(bool branch, int score, vector<NodeID> nodeChain, pair<size_t, size_t> nodePos, SearchRes correctionStatus );
        ...
        void validateSeeds( vector<Seed>& mergedSeeds, vector<NodePosPair>& nppv);
...
public:
        ...
        ReadCorrection(const DBGraph& dbg_, const Settings& settings_,
                          const sparseSA& sa_, const std::vector<long>& startpos_ , MarkovChainHandler & mch_, bool markov_filter_, bool branchAndBound_ , bool essaMem_) :
                          dbg(dbg_), settings(settings_),
                          alignment(2, 1, -1, -3), sa(sa_), startpos(startpos_), mch(mch_) , graphaln(dbg_, mch_,settings.getReadCorrDFSNodeLimit()), markov_filter(markov_filter_), branchAndBound(branchAndBound_),  useEssaMem (essaMem_){}
        ...
        void correctChunkInOneStep(vector<ReadRecord>& readChunk,
                                     AlignmentMetrics& metrics);
...
private:
        ...
        void workerThreadSingleStep(size_t myID, LibraryContainer& libraries,
                                         AlignmentMetrics& metrics);
...
public:
         ...
         * Default constructor
         */
        ReadCorrectionHandler(DBGraph& g, const Settings& s,  bool markov_filter ,  bool useEssaMem_);
        ...
        /**
         * correct errors in the reads in one step intead of using two step MM 
         * 
         */
        void doErrorCorrectionSingleStep(LibraryContainer& libraries);
