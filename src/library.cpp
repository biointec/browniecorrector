                                 ...
                                 size_t& chunkOffset, size_t targetChunkSize)
{
        ...
        for (size_t i = nextChunkOffset; i < recordBuffer.size(); i++) {
                ...
                if (thisChunkSize >= targetChunkSize && buffer.size() % 2 == 0)
                        break;
        }
        numChunksRead++;
}
                               ...
                               size_t& chunkOffset, size_t targetChunkSize)
{
        ...
        for (size_t i = nextChunkOffset; i < recordBuffer.size(); i++) {
                ...
                if (recordBuffer[i].getReadLength() >= Kmer::getK() )
                        thisChunkSize +=   recordBuffer[i].getReadLength() + 1 - Kmer::getK();
...
void LibraryContainer::inputThreadLibrary(ReadLibrary& input)
{
        ...
        while (readFile->good()) {
                ...
                while ((thisBlockSize < targetBlockSize || totNumReads % 2) && (readFile->getNextRecord(record))) {
                        thisBlockNumRecords++;
                        totNumReads++;
                        ...
                        if (thisChunkSize >= targetChunkSize && totNumReads % 2 == 0) {
                                thisBlockNumChunks++;
                                thisChunkSize = 0;
                        }
                }
