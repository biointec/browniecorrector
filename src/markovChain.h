/************************************************************************************
*    Copyright (C) 2017-2018 Mahdi hydari (mahdi.heydari@ugent.be)                  *    
*                                                                                   *
...
const size_t maxMCOrder = 15;
const size_t minMCOrder = 1;
const double illuminaErrorRate =.01 ;
const double minimumExpectedCov = 5;
const double minOddratioLog = 1;
const double errorMean = 1;
...
public :
        ...
        bool equalEndLonger(vector<NodeID> nodeList);
        
        bool removeDecision (NodeID nodeID);
        size_t getLength(){
                return length_avg;
        }
...
public :
        ...
        void forceRemove(vector<NodeID> nodeChain);
         ...
         void forceRemoveDown(vector<NodeID> nodeChain);
        /** 
...
private:
         ...
         *
         */ 
        bool searchInMCTable(vector<NodeID> nodeChain,  ChainRow &row) const;
        /**
        ...
        void dfs(NodeID currNode, NodeID nextNode, vector <NodeID> &nodeChain , vector <vector <NodeID>>& chains);
public:
         ...
         */
        set<NodeID>  getPotentialNextNodes(vector<NodeID> nodeChain) const;
        ...
        /**
         * find connections between all possible chains in the graph to make bigger contigs
         */
        
        void concatenateChains();
        
        bool validateChain(vector<NodeID> nodeChain , vector< vector<NodeID> > &ValidSubchains);
