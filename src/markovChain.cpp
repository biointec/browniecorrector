...
/************************************************************************************
*    Copyright (C) 2017-2018 Mahdi Heydari (mahdi.heydari@ugent.be)                 *    
*                                                                                   *
...
bool MarkovChainHandler::searchInMCTable(vector<NodeID> nodeChain, ChainRow &row) const{
...
set<NodeID>  MarkovChainHandler::getPotentialNextNodes(vector<NodeID> nodeChain) const{
...
size_t MarkovChain::getChainLength(vector<NodeID> nodeChain){
        ...
        len = Kmer::getK() ; // for the first and the last item in the chain
        for (size_t k = 0; k<nodeChain.size(); k++){
                SSNode node = dbg.getSSNode(nodeChain[k]);
                len = len + node.getMarginalLength();
        }
        return len;
...
bool ChainRow::makeDecision( float expectedCov){
        ...
        for (size_t i = 0 ; i < decisionVector.size (); i++){
                ...
                long double oddsratio = probMul_0 /probMul_1 ;

                if (log10(oddsratio) > minOddratioLog && decisionVector[i].getFrequency() < expectedCov){
                        /*#ifdef DEBUG
                        printRow();
                        ...
                        decisionVector[i].setOddRatioOfAcceptance(oddsratio);
...
void ChainRow::printRow (){
        ...
        for ( size_t i=0; i<decisionVector.size();i ++)
        {
                ...
                << " ) , prob( " << float ((float)nextNodes.getFrequency() /(float) totalSampleSize) << " )" <<endl;
...
bool ChainRow::equalEndLonger(vector<NodeID> nodeList){
        size_t minSize = nodeList.size() ? nodeList.size() <  nodeChain.size() :nodeChain.size();
        vector<NodeID> shortList = nodeChain;
        vector<NodeID> longList  = nodeList;
        if (nodeChain.size() == minSize){
                shortList = nodeList;
                longList = nodeChain;
                
        }
        for (size_t i = 0; i < minSize; i++){
                if (longList[longList.size()-shortList.size() + i] != shortList[i]){
                        return false;
                }
        }
        return true;
...
bool ChainRow::removeDecision (NodeID nodeID){
        size_t index = 0;
        bool remove = false;
        for (auto it : decisionVector){
        ...
        if (remove ){
                decisionVector.erase(decisionVector.begin()+index);
                return true;
        }
        return false;
}
...
void TransitionTable::forceRemoveDown(vector<NodeID> nodeChain){
        for (vector<ChainRow>::iterator it= rows.begin(); it != rows.end(); )
        {
                
                if(it->equalEndLonger (nodeChain))
                        it = rows.erase(it);
                else
                        ++it;
        }
...
void MarkovChainHandler::concatenateChains(){
        for (NodeID id = -numNodes; id <= numNodes; id++) {
                if (id ==0)
                        continue;
                ...
                for (size_t order = maxMCOrder; order >=minMCOrder ; order--){
                        if (!tc.orderTable[order-minMCOrder].modified)
                                continue;
        ...
        for (NodeID id = -numNodes; id <= numNodes; id++) {
                ...
                for (size_t order = maxMCOrder; order >=minMCOrder ; order--){
                        if (!tc.orderTable[order-minMCOrder].modified)
                                continue;
                        // remove chain with the same tail but shorter
                        for (size_t i = 0; i <tc.orderTable[order-minMCOrder].rows.size(); i++){
                                ChainRow& row = tc.orderTable[order-minMCOrder].rows[i];
                                for (size_t j = minMCOrder ; j < order ; j++){
                                        tc.orderTable[j-minMCOrder].forceRemoveDown(row.nodeChain);
                                }
                                // remove even this chain if it dosn't have any correct decision
                                bool deleteRow = true;
                                for ( size_t i=0; i<row.decisionVector.size();i ++)
                                {
                                        Decision nextNode = row.decisionVector[i];
                                        if (nextNode.getDecision())
                                                deleteRow = false;
                                }
                                if (deleteRow)
                                        tc.orderTable[order-minMCOrder].forceRemoveDown(row.nodeChain);
                                else{
                                        //remove those decision which are not true
                                        for ( size_t i=0; i<row.decisionVector.size();i ++)
                                        {
                                                Decision nextNode = row.decisionVector[i];
        
                                                if (!nextNode.getDecision())
                                                        row.removeDecision(nextNode.getNodeID());
        ...
        for (NodeID id = -numNodes; id <= numNodes; id++) {
                ...
                for (size_t order = maxMCOrder; order >=minMCOrder ; order--){
                        if (!tc.orderTable[order-minMCOrder].modified)
                         ...
                         for (size_t i = 0; i <tc.orderTable[order-minMCOrder].rows.size(); i++){
                                ChainRow row = tc.orderTable[order-minMCOrder].rows[i];
                                if (row.nodeChain.size()>1){
                                        for (size_t i = 0; i< row.decisionVector.size(); i++){
                                                vector <NodeID> chain  = row.nodeChain;
                                                chain.push_back (row.decisionVector[i].getNodeID());
                                                NodeID curr = chain[0];
                                                NodeID next = chain[1];
                                                dfs(curr,next, chain, chains);  
                                        }
                                }else{
                                        for (size_t  i = 0; i< row.decisionVector.size(); i++){
                                                vector <NodeID> chain;
                                                chain.push_back (row.nodeChain[0]);
                                                chain.push_back (row.decisionVector[i].getNodeID());
...
void MarkovChainHandler::dfs(NodeID currNode, NodeID nextNode, vector <NodeID> &nodeChain, vector <vector <NodeID>>& chains){
        TableContainer tc = markovTable[currNode + numNodes];
        if (!tc.modified){
                chains.push_back(nodeChain);
                return;
        }
        if (nodeChain.size()>10000){
                chains.push_back(nodeChain);
                cout << "exeed size limit" <<endl;
                return;
        }
        ...
        for (size_t order = maxMCOrder; order >=minMCOrder ; order--){
                if (!tc.orderTable[order-minMCOrder].modified)
                        continue;
                found = true;
                for (size_t k = 0; k <tc.orderTable[order-minMCOrder].rows.size(); k++){
                        ChainRow row = tc.orderTable[order-minMCOrder].rows[k];
                        for ( size_t d=0; d<row.decisionVector.size();d ++)
                        {
                                if (row.decisionVector[d].getNodeID() == nextNode){
                                        vector <NodeID> chain  = row.nodeChain;
                                        if (chain.size()>1){
                                                chain.pop_back();
                                                nodeChain.insert(nodeChain.begin(), chain.begin(), chain.end());
                                                NodeID curr = nodeChain[0];
                                                NodeID next = nodeChain[1];
                                                size_t numOfLoops = 0;
                                                for (size_t c = 1; c < nodeChain.size(); c++){
                                                        if (nodeChain[c] == curr ){
                                                                numOfLoops++;
                                                                if (numOfLoops >5){
                                                                        chains.push_back(nodeChain);
                                                                        //printChain(nodeChain);
                                                                        return; // loop detected.
                                                                }
                                                        }
                                                }
                                                dfs(curr, next, nodeChain, chains); 
...
bool MarkovChainHandler::validateChain(vector<NodeID> chain, vector< vector<NodeID> > &validSubchains){
        vector<NodeID> originalChain = chain;
        size_t i = chain.size()-1;
        bool valid = true;
        vector<NodeID>  subChain;
        validSubchains.push_back(subChain);
        while ( i >= minMCOrder ){
                ...
                if (nextNodes.find(next) == nextNodes.end()){
                        ...
                        if (i == minMCOrder){
                                vector<NodeID>  subChain;
                                validSubchains.push_back(subChain);
                                validSubchains.back().insert(validSubchains.back().begin(), chain.begin(), chain.end()); 
                        }
                }
                else{
                        validSubchains.back().insert(validSubchains.back().begin(), next);
                        if (i == minMCOrder){
                                validSubchains.back().insert(validSubchains.back().begin(), chain.begin(), chain.end()); 
                        }
                }
        ...
        std::sort(validSubchains.begin(), validSubchains.end(), [](const vector<int> & a, const vector<int> & b){ return a.size() > b.size(); });
        return valid;
}
